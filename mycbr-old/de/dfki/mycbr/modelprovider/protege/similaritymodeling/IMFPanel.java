...
public class IMFPanel extends javax.swing.JPanel implements HasChangedListenerSMF {
	/**
	 * 
	 */
	...
	JPanel paIMF = new JPanel();
	//TitledBorder titledBorder1;
	GridBagLayout gridBagLayout1 = new GridBagLayout();

	private ModelCls modelCls;
	private IMF_Standard imf;
	...
	private void customInit() {
		...
		buClose.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				log.fine("pressed CLOSE"); //$NON-NLS-1$
		...
		imf = CBRProject.getInstance().getIMFforModelCls(modelCls.getName());
		imf.checkConsistency(false);
		
		// commented out before 20.10.2008
//		log.info("IMF for "+modelCls+" == null? "+(imf==null) );
//		currentIMFunction.removeHasChangedListener(this);
//		currentIMFunction.addHasChangedListener(this, false);
		
		paIMF.removeAll();
		// then show its panel
		JPanel panel = imf.getIMFPanel();
		paIMF.add(panel, BorderLayout.CENTER);
...
//	}
	...
	private void jbInit() throws Exception {
		...
		paIMF.setBorder(BorderFactory.createEtchedBorder());
		paIMF.setLayout(borderLayout2);
		...
		this.add(paIMF, new GridBagConstraints(0, 1, 2, 1, 1.0, 1.0, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 396, 96));
	...
	public String toString() {
		return String.format(Messages.getString("Holder_panel_for"), imf); //$NON-NLS-1$
	}
