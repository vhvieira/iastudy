...
public class ResultsTableModel extends AbstractTableModel implements TableModel {
	...
	private final static Logger log = Logger.getLogger(ResultsTableModel.class
			.getName());
	...
	public ResultsTableModel(DefaultMutableTreeNode root, DefaultQuery query,
	...
	public Object getValueAt(Object _node, int column) {
		...
		if (column == 0) {
			...
			if (value instanceof DefaultQuery)
				value = null;
			return value;
		} else if (column-1 < caseInstances.length) {
			// CASE VALUE
	...
	public void setValueAt(Object value, Object _node, int column) {
		...
		// if (column-1 < caseInstances.length)
		{
			// CASE VALUE
			log.info("Setting case value by table is currently not provided.");
		}
	}
